...
- [x] `RandomBot`（第一天就该有的 fuzzer）与 `GreedyBot`，都只从 `legalActions` 取动作。
- [x] `bun run play --seed 1 --p0 greedy --p1 random` —— 终端跑完整一局，逐步打印状态与事件。
- [x] `bun run replay <file> --step` —— 回放器，排 bug 的主力工具。
- [x] `sim`：批量对打 + 每步不变量断言
  - [x] 血量非负
  - [x] 单实体不跨区
  - [x] **槽位无重复占用**
...
- [x] 每个 replay 文件**内嵌完整 `RulesConfig` 快照**，回放时用文件里的配置，
      **不读默认值**。否则改一次默认值，历史 golden 集体变红，而它们本该是不变量。
- [x] golden 集**至少一局跑 `first_passer`**（上表最后一行）。
