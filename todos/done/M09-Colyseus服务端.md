...
- [x] `src/transport/` 隔离层：定义我们自己的 `Transport` 接口，Colyseus 的 API
      只出现在这一层之内（架构文档 §1.2 的风险隔离要求）。
- [x] `MatchRoom`：座位分配、`intent` 收发、`seq`、快照/事件下发（**逐客户端投影后**广播）。
- [x] 计时：用 `this.clock`（可暂停时钟），**每 action 一个计时器**（默认 30 秒），
      超时视同 pass——**不是每回合一个计时器**。
- [x] 部署相位：服务端聚合双方的秘密选择，凑齐后以**单个** `deploy` intent 喂引擎。
- [x] 重连：`allowReconnection`（**它返回 Promise，忘了 catch 会静默吞掉判负逻辑**，
      框架 §13 坑 7），重连后补发全量快照。
- [x] 认输、超时判负、对局结果落库（走 `onDispose` 或独立队列，别阻塞房间销毁）。
- [x] **`ResolutionLoopError` 撞上时那一局怎么办**（M5/T4 留下的残余风险，需要在这里拍板）。
...
- [x] 服务端收到意图**重新完整校验**（M7 的 `legal` 字段不是权威）。
- [x] 玩家身份用**不透明 `playerId` 字符串**，不要假设「匿名 = 无 id」，
