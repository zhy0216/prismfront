...
- [x] **M10a** `Transport` 抽象 + `MockTransport`、Boot/Match 场景、`Layout`、
      棋盘与单位渲染、卡面程序化合成
  - 验收：加载 `?replay=deploy-r1-r2` 能看到正确棋盘
- [x] **M10b** `Director` + Beat 编排（齐射 / 死亡 / 空格穿透光束）
  - 验收：七个 golden replay 逐个播放正确
- [x] **M10c** `HudScene`：**光源条**、水晶、基地血、复燃泉、每 action 计时环
  - 验收：`color-gate-blackout` 播放时灯会灭并重亮
- [x] **M10d** 输入状态机 + `OverlayScene`（换牌 / 部署 / 发现 / 选目标）
  - 验收：热座模式能出牌
- [x] **M10e** 接 `ColyseusTransport`、`seq` 校验、`resync`、`fastForward`
...
- [x] 编排层零 Phaser 依赖（可 `bun test`，且 Phaser 4 出问题能回退 3.90）
- [x] 每个 Beat 都实现 `complete()`（跳过 / 重连 / 切后台都靠它）
- [x] **只翻上下，不翻左右**（客户端设计 §4.2）——索引轴对双方一致，
      否则斜打方向在两人屏幕上相反
- [x] **卡面立绘从第一天就走同一条渲染路径**（[决策 #6](./决策待办.md) 已拍板美术推迟）：
