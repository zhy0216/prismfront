...
- [x] 按《数值基准》§6 的 33 张基准卡表建卡，ID 用 `PF1_R01` 式（《命名与主题》§4）。
      §6.1 已经用 v1.1 公式回查过示例卡并给出修正值：
  - [x] 斜刺长枪兵改 **4/3 + 斜打**
  - [x] 荆棘卫士改 **Retaliate 2**
  - [x] 战地号手欠 3 点 → 示例卡 2/4 改 **3/4**（不进卡池，§6.1 结论 + v2 §8.5 已同步）
- [x] 三名首发英雄：红 5/4、绿 4/6、蓝 **3/6（纯身板，无技能）**。
...
- [x] **专属卡归属**（[决策 #11](./决策待办.md)，规范 §11.1/§11.4b，2026-08-08 补订）：
...
- [x] `RulesConfig.deck.maxCopies = **3**`（[决策 #12](./决策待办.md) 取代
...
- [x] **L3 语义校验**上线：
  ...
  - [ ] v2 §9 的六条新增校验
- [x] **色轮越权 lint**：数据来源就是 M1 建的 `COLOR_OWNERSHIP` 常量。
      红卡出现 `act.swap` → 报错。
...
- [x] 资源上限表（单卡 512 节点、深度 32、拦截链 8、单卡 64KB…）。
- [x] `ir:print` / `ir:diff` 完成。
