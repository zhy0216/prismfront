...
//
// 为什么 engine 需要自己的一份：本包 tsconfig 是 `types: []`，且 turbo boundaries 的
// `pure` tag 明确 deny 了 `@types/bun`（架构 §2.2 禁令 5：packages/* 必须运行时中立）。
// 每个包的 `tsc --noEmit` 是**独立的 program**（架构 §4.3 不用 project references），
// 且 ir 的这份垫片没有被任何 `import` 图触达，所以 engine 的 program 看不到它——
// engine 的 *.test.ts 写 `import { test } from "bun:test"` 会报 TS2307。
// 而 packages/engine/biome.json 的 overrides 又专门为测试文件放行了 `bun:test`
// —— 说明测试就该写在这里。缺的只是类型。
//
