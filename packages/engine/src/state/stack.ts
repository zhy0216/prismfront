 ...
 * 例：`"CORE_050#play.1"`。
 *
 * 栈条目**不内联卡牌节点**而用引用，收益是条目极小 → `clone(state)` 快 → MCTS 可行、
 * 快照/存档/回放便宜。前提是 `state.bundleId` 在对局开始时钉住（IR v1 §2.1 / §6.2），
 * 否则热更会让 ref 指向错的节点。
 */
 ...
 * 栈条目：内联的 IR 动作节点。
 *
 * 属于 IR v1 §5.6 的**运行时超集** —— 由引擎自己生成、永不来自外部输入。
 * 引擎生成的动作在 bundle 里没有位置，因此无 ref 可用，只能内联：
 * - 战斗快照逐条应用出的 `act.strike` / `act.hit`（v2 §4.2 第 ③ 步，M3）；
 * - 拦截器改写后重新入栈的动作（M5）；
 * - M2 手写临时 handler 跑通「抽牌 → 放单位到格 → 手动 strike → 死亡」用的动作。
 *
