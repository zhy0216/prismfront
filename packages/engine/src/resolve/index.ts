...
// - **英雄的阵亡一侧已经完整**（M6）：`deaths.ts` 的 `sendOffBoard` 按卡面 `kind`
//   把英雄送进 `fountain` 而不是墓地、回写 `respawnAt`、并发 `hero_died` 而**不发**
//   `unit_died`（v2.1 §11.3 要求触发器区分两者）；`while_source_alive` 的剥离判据
//   随之认得复燃泉。判据只有 `eval/context.ts` 的 `isHero` 一处，
//   不接卡表时整块退化成"英雄按随从结算"，与 M2~M5 逐字相同。
//   复活的**消费**侧（到期的英雄怎么拿到部署名额、`damage` 与附魔在哪一步清）
//   在 `rules/phase.ts` —— 本目录只负责把它送进泉并写下回来的回合，**什么都不清**。
// - **不含**：投影与 legalActions（M7）。
